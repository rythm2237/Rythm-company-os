...
import { AGENT_OAUTH_COOKIE_NAMES, type AgentOAuthProviderKey, verifyAgentOAuthState } from "@/lib/integrations/connections/agent-oauth";
...
export async function maybeHandleAgentOAuthCallback(request: Request, providerKey: AgentOAuthProviderKey) {
  ...
  if (
    !code || jar.get("rythm_agent_oauth_state") !== state || jar.get("rythm_agent_oauth_session") !== payload.sessionId ||
    jar.get("rythm_agent_oauth_integration") !== payload.integrationId || jar.get("rythm_agent_oauth_user") !== payload.userId ||
    jar.get("rythm_agent_oauth_provider") !== providerKey || !jar.get("rythm_agent_oauth_pkce")
  ...
  try {
    ...
    const stored = await service.rpc("store_organization_integration_secret_unverified_v1", { target_integration_id: payload.integrationId, secret_value: envelope });
    ...
    const updated = await service.from("organization_integrations").update({
      ...
      metadata: {
        ...
        oauth_flow: "connection_agent_oauth_v1",
    ...
    await service.from("audit_events").insert({
      ...
      risk_level: "low",
      payload: { auth_type: "oauth", resource_count: resources.length, connection_agent_session_id: payload.sessionId },
    });
