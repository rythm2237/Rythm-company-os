...
type KnowledgeAgent = {
  ...
  company_knowledge_connected?: boolean | null;
};
...
function roleCanUse(item: KnowledgeRow, agent: KnowledgeAgent) {
  const role = normalize(agent.role_title); const department = normalize(agent.department);
  const explicitDepartments = item.allowed_departments ?? []; const explicitRoles = item.allowed_role_keywords ?? [];
...
async function fileFromKnowledge(context: KnowledgeContext, item: KnowledgeRow): Promise<AgentAttachmentInput | null> {
  try {
    let buffer: Buffer | null = null; let mimeType = item.mime_type || "application/octet-stream";
    let filename = item.title.replace(/[^a-zA-Z0-9._ -]+/g, "-").slice(0, 120) || "company-reference";
    if (item.storage_path) {
      const { data, error } = await context.supabase.storage.from("company-knowledge").download(item.storage_path); if (error || !data) return null;
      buffer = Buffer.from(await data.arrayBuffer()); if (data.type) mimeType = data.type;
    } else if (item.source_url && item.mime_type && /^(image\/|application\/pdf)/.test(item.mime_type)) {
      const response = await fetch(item.source_url, { signal: AbortSignal.timeout(8000), redirect: "follow" }); if (!response.ok) return null;
      buffer = Buffer.from(await response.arrayBuffer()); mimeType = response.headers.get("content-type")?.split(";")[0] || mimeType;
...
export async function loadCompanyKnowledgeForAgent(context: KnowledgeContext, agent: KnowledgeAgent, currentTask = "") {
  ...
  if (agent.company_knowledge_connected === false) {
    return {
      contextText: [professional.contextText, "LIVE COMPANY KNOWLEDGE — DETACHED FOR TRANSFER. No company facts, assets or company-scoped knowledge are available to this Agent until an authorized organization owner reconnects it."].filter(Boolean).join("\n\n--- KNOWLEDGE LAYER BOUNDARY ---\n\n"),
      attachments: [] as AgentAttachmentInput[], knowledgeCount: 0, professionalFoundation: professional.foundationTitle,
      specializationTitles: professional.specializationTitles, professionalQaRules: professional.qaRules, companyKnowledgeConnected: false,
    };
  }
  ...
  const entries = relevant.filter((item) => item.content?.trim() || item.source_url).map((item) => {
    const source = item.source_url ? ` Source reference retained internally: ${item.title}` : "";
