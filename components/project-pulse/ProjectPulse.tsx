...
type PulseEvent = { id:string; event_type:string; event_label:string; previous_progress:number; new_progress:number; previous_node:string|null; new_node:string|null; event_state:"completed"|"current"|"upcoming"|"blocked"|"waiting_approval"; next_step:string|null; };
...
export default function ProjectPulse({event,nodes,project}:Props){
  ...
  const animate=()=>{
    if(!event)return;
    setProgress(event.previous_progress); setTokenProgress(0); setDone(false); setOpen(true);
    const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if(reduced){setProgress(event.new_progress);setTokenProgress(1);setDone(true);return;}
    const delta=Math.abs(event.new_progress-event.previous_progress); const distance=Math.max(1,newIndex-prevIndex);
    const duration=Math.min(3000,Math.max(1500,1500+distance*220+delta*24)); const started=performance.now();
    const tick=(now:number)=>{const t=Math.min(1,(now-started)/duration); const eased=1-Math.pow(1-t,3); setTokenProgress(eased); setProgress(Math.round(event.previous_progress+(event.new_progress-event.previous_progress)*eased)); if(t<1) requestAnimationFrame(tick); else {setProgress(event.new_progress);setTokenProgress(1);setDone(true);}};
  ...
  },[event?.id,project?.project_code,ordered.length]);

  if(!event||!project||!ordered.length)return null;
  const close=()=>{window.localStorage.setItem(`rythm-project-pulse:${event.id}`,"seen");setOpen(false);};
  if(!open) return <button className={styles.launcher} onClick={animate} aria-label={`Open Project Pulse for ${project.name}`}><span>Project Pulse</span><strong>{event.new_progress}%</strong></button>;
  ...
  const nodeState=(node:PulseNode)=>{const i=ordered.findIndex(n=>n.stage_code===node.stage_code); if(event.event_state==="blocked"&&i===newIndex)return "blocked"; if(event.event_state==="waiting_approval"&&i===newIndex)return "waiting_approval"; if(i<newIndex)return "completed"; if(i===newIndex)return done?"current":"upcoming"; return "upcoming";};
  ...
  return <div className={styles.backdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="pulse-title">
    <div className={styles.header}><div><p className="eyebrow">RYTHM PROJECT PULSE</p><h2 id="pulse-title">{event.event_label}</h2><p>{project.project_code} · {project.name}</p></div><button className={styles.close} onClick={close} aria-label="Close Project Pulse">×</button></div>
    <div className={styles.summary}><div><span>Previous</span><strong>{event.previous_progress}%</strong></div><div className={styles.current}><span>Project progress</span><strong aria-live="polite">{progress}%</strong></div><div><span>Destination</span><strong>{event.new_progress}%</strong></div></div>
    ...
    <div className={styles.footer}><div><span className={styles.kicker}>Transition</span><strong>{ordered[prevIndex]?.label??event.previous_node} → {ordered[newIndex]?.label??event.new_node}</strong></div><div><span className={styles.kicker}>Next governed step</span><strong>{event.next_step??"Continue under project governance."}</strong></div><button className={`primary-link ${styles.continue}`} onClick={close}>{done?"Continue":"View progress"}</button></div>
  </section></div>;
}
