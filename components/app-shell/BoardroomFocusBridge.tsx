...
export default function BoardroomFocusBridge() {
  ...
  return <>
    <style jsx global>{`
      ...
      .boardroom-leave-room { position: fixed; z-index: 2147483605; top: 10px; left: 60px; height: 38px; padding: 0 14px; border: 1px solid rgba(255,255,255,.16); border-radius: 11px; background: rgba(17,21,29,.92); color: #edf2fb; font: 700 12px/1 system-ui,sans-serif; backdrop-filter: blur(14px); box-shadow: 0 10px 30px rgba(0,0,0,.28); cursor: pointer; }
      .boardroom-leave-room:hover { background: rgba(30,37,50,.98); }
      .boardroom-leave-room:disabled { opacity: .55; cursor: wait; }
      .boardroom-leave-error { position: fixed; z-index: 2147483606; top: 56px; left: 60px; max-width: 360px; padding: 9px 11px; border: 1px solid rgba(255,113,132,.34); border-radius: 10px; background: rgba(56,17,24,.96); color: #ffc4cc; font: 600 11px/1.4 system-ui,sans-serif; }
      @media (max-width: 760px) { .boardroom-leave-room { left: 52px; top: 8px; height: 34px; padding: 0 10px; font-size: 10px; } .boardroom-leave-error { left: 8px; right: 8px; top: 48px; max-width: none; } }
    `}</style>
