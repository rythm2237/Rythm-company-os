...
export async function POST(request:Request) {
  ...
  catch{return jsonError("The active organization does not have an active AI entitlement.",403);}
  ...
  if (session.status === "ready") {
    ...
    const { data:claimed, error:claimError } = await supabase.from("meeting_agent_sessions")
      .update({ status:"running", started_by_user_id:user.id, started_at:startedAt, model:"adaptive-routing", error_message:null, updated_at:startedAt })
    ...
    await supabase.from("audit_events").insert({ organization_id:organizationId, actor_type:"user", actor_user_id:user.id, event_type:"meeting.agent_deliberation_started", object_type:"meeting", object_id:meeting.id, risk_level:"medium", payload:{ session_id:sessionId, participants:participants.length, rounds:session.max_rounds, routing_feature:"boardroom.deliberation", routing_policy:"adaptive", external_actions:false, external_research:false } });
  }
  ...
  const runDeliberationGateway=async(agent:AgentRow,systemInstructions:string,prompt:string,currentCost:number,maxOutputTokens:number)=>executeAiRequest({
    organizationId,
    actor:{type:"agent",userId:user.id,agentId:agent.id},
    context:{meetingId:meeting.id,meetingSessionId:sessionId,projectId:session.project_id},
    feature:"boardroom.deliberation",
    systemInstructions,
    prompt,
    ...
    agentPolicy:buildProductionAgentPolicy({
      ...
      riskCeiling:"high",
      maxCostPerRequest:effectiveRequestCostLimit(entitlement!,Math.max(0,Number(session.budget_cap_usd??0)-currentCost)),
      maxOutputTokens,
  ...
  try {
    ...
    if(lastMessage?.message_type==="ceo_contribution" && deliberationMessages.length>=totalAgentTurns){
      ...
      const gateway=await runDeliberationGateway(synthesizerAgent,`You are ${synthesizerAgent.agent_code}, ${synthesizerAgent.display_name??synthesizerAgent.name}, responding inside an open Human CEO-chaired meeting. Address the Human CEO's latest contribution directly, correct misunderstandings where needed, and identify whether the prior recommendation should change. Do not close the meeting. Do not claim external research. Respond in ${session.language}. Use short headings: Response to Chair; Clarification; Updated Recommendation.`,`${sharedContext}\n\nTranscript including the latest Human CEO contribution:\n${transcript}`,Number(session.estimated_cost_usd??0),1200);
      const responseText=gateway.outputText.slice(0,6000);
    ...
    if (deliberationMessages.length < totalAgentTurns) {
      ...
      if (!responseText) {
        const reason = "empty Gateway output";
      ...
      const { error:insertError } = await supabase.from("meeting_agent_messages").insert({ organization_id:organizationId, meeting_id:meeting.id, session_id:sessionId, agent_id:responseAgent.id, turn_index:nextTurnIndex, round_no:roundNo, speaker_type:"agent", message_type:messageType, content:responseText, model:gateway.routingDecision.selectedModel, ai_correlation_id:gateway.correlationId, input_tokens:inputTokens, output_tokens:outputTokens, estimated_cost_usd:cost });
      if (insertError) {
    ...
    const { error:synthesisInsertError } = await supabase.from("meeting_agent_messages").insert({ organization_id:organizationId, meeting_id:meeting.id, session_id:sessionId, agent_id:responseAgent.id, turn_index:nextTurnIndex, round_no:roundNo, speaker_type:"agent", message_type:"synthesis", content:responseText, model:gateway.routingDecision.selectedModel, ai_correlation_id:gateway.correlationId, input_tokens:inputTokens, output_tokens:outputTokens, estimated_cost_usd:cost });
