...
export async function POST(request: Request) {
  ...
  if (!connection.data || connection.data.provider_key !== "youtube") {
    return back(request, integrationId, "This YouTube connection is not valid for the active company.");
  }

  const { clientId, clientSecret } = credentials();
  if (!clientId || !clientSecret) return back(request, integrationId, "Google OAuth platform credentials are not configured.");
  ...
  await context.supabase.from("audit_events").insert({
    ...
    risk_level: "low",
    payload: { provider_key: "youtube", redirect_uri: redirectUri, scope: "youtube.readonly" },
  });
  ...
  response.cookies.set("rythm_yt_state", state, opts);
  response.cookies.set("rythm_yt_integration", integrationId, opts);
  response.cookies.set("rythm_yt_user", context.user.id, opts);
  response.cookies.set("rythm_yt_pkce", verifier, opts);
  return response;
}
