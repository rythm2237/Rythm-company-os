...
const GOOGLE_ADS_SCOPES = ["openid", "email", GOOGLE_ADS_SCOPE];
...
export async function POST(request: Request) {
  ...
  const { data: integration } = await context.supabase
    ...
    .maybeSingle();
  if (!integration || integration.provider_key !== "google_ads") {
    return back(request, integrationId, projectId, "This Google Ads connection is not valid for the active company.");
  }
  ...
  consentUrl.searchParams.set("scope", GOOGLE_ADS_SCOPES.join(" "));
  ...
  await context.supabase.from("audit_events").insert({
    ...
    risk_level: "low",
    payload: { provider_key: "google_ads", project_id: projectId, scope: "adwords" },
  });

  const response = NextResponse.redirect(consentUrl, 303);
  const cookieOptions = { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax" as const, path: "/api/integrations/google-ads", maxAge: 10 * 60 };
  response.cookies.set("rythm_google_ads_oauth_state", state, cookieOptions);
  response.cookies.set("rythm_google_ads_oauth_integration", integrationId, cookieOptions);
  response.cookies.set("rythm_google_ads_oauth_user", context.user.id, cookieOptions);
