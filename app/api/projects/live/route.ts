...
async function projectContext(projectId:string){
  ...
  const project=await auth.supabase.from("projects")
    .select("id,name,project_code,status,stage,progress_percent,last_heartbeat_at")
...
export async function GET(request:Request){
  ...
  const counts={
    ...
    queued:tasks.filter((t:any)=>["queued","retrying"].includes(t.status)).length,
    waitingApproval:tasks.filter((t:any)=>t.status==="waiting_for_approval").length,
    waitingOther:tasks.filter((t:any)=>["waiting_for_connection","waiting_for_data"].includes(t.status)).length,
    completed:tasks.filter((t:any)=>t.status==="completed").length,
    failed:tasks.filter((t:any)=>["failed","blocked"].includes(t.status)).length,
    total:tasks.length,
  };
...
export async function POST(request:Request){
  ...
  if(responseNote.length<3)return NextResponse.json({ok:false,error:"Add a short CEO note before resolving this request."},{status:400});
  const context=await projectContext(projectId);if(!context.ok)return context.response;
  const {auth}=context;
  const approvalResult=await auth.supabase.from("approval_requests")
    .select("id,subject_type,subject_id,title,risk_level,status,expires_at")
    .eq("id",approvalId).eq("project_id",projectId).eq("organization_id",auth.organizationId).maybeSingle();
  ...
  const resolved=await auth.supabase.from("approval_requests").update({status:resolution,response_note:responseNote,resolved_at:resolvedAt,approver_user_id:auth.user.id})
    .eq("id",approvalId).eq("project_id",projectId).eq("organization_id",auth.organizationId).eq("status","pending").select("id").maybeSingle();
  ...
  await auth.supabase.from("audit_events").insert({
    organization_id:auth.organizationId,actor_type:"user",actor_user_id:auth.user.id,event_type:`approval.${resolution}`,
    object_type:"approval_request",object_id:approvalId,risk_level:approval.risk_level,
    payload:{title:approval.title,resolution,response_note:responseNote,human_authority:"Human CEO / Owner",source:"project_operating_view",resolved_at:resolvedAt}
  });
  await auth.supabase.from("project_activity_events").insert({
    organization_id:auth.organizationId,project_id:projectId,event_type:`approval.${resolution}`,
    headline:`CEO ${resolution}: ${approval.title}`,detail:responseNote,importance:resolution==="approved"?"major":"attention"
  });
