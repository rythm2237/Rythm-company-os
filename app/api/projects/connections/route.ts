...
export async function POST(request:Request){
  ...
  const result=await auth.supabase.from("project_connection_bindings").upsert({organization_id:auth.organizationId,project_id:projectId,integration_id:integrationId,resource_type:String(body.resourceType??integration.data.provider_key).trim(),resource_ref:resourceRef,display_name:String(body.displayName??resourceRef).trim().slice(0,240),permission_scope:{capabilities:requested},access_status:"connected",recommendation_level:"confirmed",confirmed_by_user_id:auth.user.id,confirmed_at:new Date().toISOString(),updated_at:new Date().toISOString()},{onConflict:"project_id,integration_id,resource_ref"}).select("id").single();
