...
export async function GET(request:Request){
  ...
  try{
    ...
    // Workstreams are deliberately fault-isolated. A Gateway/proposal/knowledge outage must never
    // freeze unrelated queued project work. Governance still applies inside each affected stream.
    const knowledgeResults=await isolated("knowledge",()=>dispatchProjectKnowledge(service,{claimLimit:4}),errors,[]);
