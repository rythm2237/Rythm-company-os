...
async function settle(client: ReturnType<typeof serviceClient>, requestId: string, response: Awaited<ReturnType<typeof executeAiRequest>>) {
  ...
  if (started.error) throw new AIWorkspaceError("USAGE_STATE_FAILED", 503);
  ...
  try {
    ...
    if (profile === "professional") {
      ...
      try {
        ...
        await client.rpc("aiw_release_usage", { p_request: activeRequest, p_error: "ENHANCEMENT_PROVIDER_FAILED" });
        activeRequest = null;
        throw error;
      }
    ...
    try {
      ...
      await client.rpc("aiw_release_usage", { p_request: activeRequest, p_error: "ANSWER_PROVIDER_FAILED" });
      activeRequest = null;
      throw error;
    }
    ...
    if (activeRequest) await client.rpc("aiw_mark_uncertain", { p_request: activeRequest, p_error: "UNEXPECTED_AFTER_RESERVATION" });
