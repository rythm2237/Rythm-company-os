...
function TrendChart({ points }: { points: Point[] }) {
  if (points.length < 2) return <div className="agency-seo-empty">Analyze the website to build its search visibility trend.</div>;
...
export default async function AgencySeoPage({ searchParams }: Props) {
  ...
  const googleBoundResource = gscResources.find((resource) => resource.id === bindingFor("google_search_console")) ?? null;
  const bingBoundResource = bingResources.find((resource) => resource.id === bindingFor("bing_webmaster")) ?? null;
  const findings = arr(latest?.findings);

  return <main className="agency-seo-shell">
    <header className="agency-seo-hero"><div><p>GROW / ADVERTISING AGENCY</p><h1>Agency SEO Client Management</h1><span>Add a client website and RYTHM will automatically match verified search accounts, collect evidence and prepare visual SEO analysis inside {organization.name}.</span></div><div className="hero-actions"><Link href="/crm">CRM & Sales</Link><Link href="/integrations">Connections</Link></div></header>
    ...
    <section className="agency-seo-layout">
      ...
      <div className="agency-seo-main">
        <section className="agency-seo-card onboarding-card"><div className="section-heading"><div><p>ADD CLIENT WEBSITE</p><h2>Start SEO management</h2></div><span>Automatic setup</span></div><form action={createAgencySeoSite} className="agency-seo-form"><label>Existing client<select name="accountId" defaultValue=""><option value="">Create a new client</option>{accounts.map((account)=><option value={account.id} key={account.id}>{account.name}</option>)}</select></label><label>New client name<input name="clientName" placeholder="Only if creating a new client"/></label><label>Website URL<input name="siteUrl" placeholder="https://client.com" required/></label><label>Website label<input name="siteName" placeholder="Main website"/></label><button type="submit">Add website</button></form><p className="provider-note">RYTHM automatically looks for matching Google Search Console and Bing Webmaster access. No property IDs or technical setup are required when a verified match already exists.</p></section>

        {selectedSite ? <>
          <section className="agency-seo-card site-heading-card"><div><p>{accountName(selectedSite.crm_account_id)}</p><h2>{selectedSite.name}</h2><a href={selectedSite.site_url} target="_blank" rel="noreferrer">{selectedSite.site_url} ↗</a></div><form action={runAgencySeoMonitoring}><input type="hidden" name="siteId" value={selectedSite.id}/><button className="run-button" type="submit">Analyze website</button></form></section>

          <section className="agency-seo-provider-grid">
            <article className="agency-seo-card"><div className="section-heading"><div><p>GOOGLE SEARCH CONSOLE</p><h3>{googleBoundResource ? "Connected automatically" : "Not connected yet"}</h3></div><span className={googleBoundResource?"status ok":"status"}>{latest ? s(google.status)|| (googleBoundResource?"ready":"not connected") : googleBoundResource?"ready":"not connected"}</span></div>{googleBoundResource?<p className="provider-note">RYTHM matched <strong>{googleBoundResource.resource_name || googleBoundResource.resource_id}</strong> to this website. No action is required.</p>:<p className="provider-note">No matching Search Console property is available yet. <Link href="/integrations">Connect Google Search Console</Link> once and RYTHM will match the website automatically.</p>}<details><summary>Advanced provider settings</summary><form action={bindAgencySeoProvider} className="binding-form"><input type="hidden" name="siteId" value={selectedSite.id}/><input type="hidden" name="providerKey" value="google_search_console"/><select name="resourceId" defaultValue={bindingFor("google_search_console")}><option value="">No property bound</option>{gscResources.map((resource)=><option value={resource.id} key={resource.id}>{resource.resource_name || resource.resource_id}</option>)}</select><button type="submit">Update</button></form></details></article>
            <article className="agency-seo-card"><div className="section-heading"><div><p>BING WEBMASTER</p><h3>{bingBoundResource ? "Connected automatically" : "Optional connection"}</h3></div><span className={bingBoundResource?"status ok":"status"}>{latest ? s(bing.status)|| (bingBoundResource?"ready":"not connected") : bingBoundResource?"ready":"not connected"}</span></div>{bingBoundResource?<p className="provider-note">RYTHM matched <strong>{bingBoundResource.resource_name || bingBoundResource.resource_id}</strong> to this website. No action is required.</p>:<p className="provider-note">Bing is not connected for this website. Google and technical analysis can still run now. <Link href="/integrations">Connect Bing Webmaster</Link> later if needed.</p>}<details><summary>Advanced provider settings</summary><form action={bindAgencySeoProvider} className="binding-form"><input type="hidden" name="siteId" value={selectedSite.id}/><input type="hidden" name="providerKey" value="bing_webmaster"/><select name="resourceId" defaultValue={bindingFor("bing_webmaster")}><option value="">No site bound</option>{bingResources.map((resource)=><option value={resource.id} key={resource.id}>{resource.resource_name || resource.resource_id}</option>)}</select><button type="submit">Update</button></form></details></article>
          </section>

          <section className="agency-seo-kpis detail"><article><span>Technical health</span><strong>{latest ? `${latest.score}/100` : "—"}</strong><small>{latest ? new Date(latest.checked_at).toLocaleString("en-GB") : "Analyze website"}</small></article><article><span>Clicks · 28d</span><strong>{latest ? fmt(totalClicks) : "—"}</strong><small>{googleDaily.length?"Google":"Bing / no Google data"}</small></article><article><span>Impressions · 28d</span><strong>{latest ? fmt(totalImpressions) : "—"}</strong><small>{latest ? `${pct(weightedCtr)} CTR` : "No snapshot"}</small></article><article><span>Avg position · 28d</span><strong>{weightedPosition ? weightedPosition.toFixed(1) : "—"}</strong><small>Impression weighted</small></article></section>
