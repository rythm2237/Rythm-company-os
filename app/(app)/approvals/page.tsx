...
const statusValues = new Set<ApprovalStatus>(["pending", "approved", "rejected", "expired", "cancelled"]);
...
async function resolveApproval(formData: FormData) {
  ...
  if (responseNote.length < 3) {
    redirect(`/approvals?approval=${approvalId}&error=CEO%20response%20note%20is%20required.`);
  }

  const { data: approval } = await supabase
    .from("approval_requests")
    .select("id, title, risk_level, status, expires_at, approver_user_id")
    .eq("id", approvalId)
    .eq("organization_id", organizationId)
    .maybeSingle();
  ...
  if (approval.status !== "pending") {
    redirect(`/approvals?approval=${approvalId}&error=Only%20pending%20requests%20can%20be%20resolved.`);
  }
  ...
  if (approval.expires_at && new Date(approval.expires_at) <= now) {
    await supabase
      .from("approval_requests")
      .update({ status: "expired", resolved_at: now.toISOString(), approver_user_id: user.id })
      .eq("id", approvalId)
      .eq("organization_id", organizationId)
      .eq("status", "pending");
    ...
    redirect(`/approvals?approval=${approvalId}&error=This%20request%20expired%20before%20resolution.`);
  }
  ...
  if (error || !resolved) {
    redirect(`/approvals?approval=${approvalId}&error=${encodeURIComponent(error?.message ?? "Approval could not be resolved.")}`);
  }
  ...
  redirect(`/approvals?approval=${approvalId}&message=Approval%20${resolution}.`);
}

export default async function ApprovalEnginePage({ searchParams }: ApprovalPageProps) {
  ...
  let approvalsQuery = supabase
    .from("approval_requests")
    .select("id, subject_type, subject_id, title, summary, risk_level, requested_by_agent_id, approver_user_id, status, conditions, response_note, expires_at, resolved_at, created_at")
    .eq("organization_id", organizationId)
    .order("created_at", { ascending: false })
    .limit(100);

  if (selectedStatus) approvalsQuery = approvalsQuery.eq("status", selectedStatus);
  if (selectedRisk) approvalsQuery = approvalsQuery.eq("risk_level", selectedRisk);
  ...
  const selectedApproval = selectedId
    ? ((await supabase
        ...
        .eq("organization_id", organizationId)
        .eq("id", selectedId)
        .maybeSingle()).data as ApprovalRow | null)
    : null;

  const audit = selectedApproval
    ? (((await supabase
        .from("audit_events")
  ...
  const conditions = selectedApproval ? conditionsToList(selectedApproval.conditions) : [];
  ...
  return (
    <main className="command-shell">
      <header className="command-header">
        <div>
          ...
          <p className="subtitle">
            Review consequential requests, impose conditions, record the CEO response, and preserve a complete audit trail.
          </p>
        </div>
      ...
      <section className="organization-banner">
        ...
        <div><span>Organization</span><strong>RYTHM</strong></div>
        <div><span>Enforcement</span><strong>Owner-only resolution</strong></div>
      </section>
      ...
      <section className="panel panel-wide" style={{ marginTop: 18 }}>
        <div className="panel-heading">
          <div><p className="label">Governance queue</p><h2>Approval Inbox</h2></div>
        ...
        <form method="get" style={{ display: "grid", gridTemplateColumns: "220px 220px auto", gap: 10, marginBottom: 18 }}>
          <select name="status" defaultValue={selectedStatus} aria-label="Filter approval status">
          ...
          <select name="risk" defaultValue={selectedRisk} aria-label="Filter risk level">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(280px, .8fr) minmax(0, 1.6fr)", gap: 18 }}>
            ...
            {approvals.length ? approvals.map((approval) => (
              ...
              >
                ...
                <span style={{ display: "block", marginTop: 6, color: "#717b8e", fontSize: ".82rem" }}>
                  {approval.risk_level} risk · {approval.status} · {formatDate(approval.created_at)}
                </span>
              </Link>
            )) : <p className="empty-state">No approval requests match these filters.</p>}
          </div>

          {selectedApproval ? (
            <article style={{ border: "1px solid #dfe4ec", borderRadius: 16, padding: 20, background: "#f8f9fb" }}>
              <div className="panel-heading">
                <div><p className="label">Request details</p><h2>{selectedApproval.title}</h2></div>
                <div className="row-meta"><span>{selectedApproval.risk_level} risk</span><b className={isPending ? "state-paused" : "state-active"}>{selectedApproval.status}</b></div>
              </div>

              <p style={{ color: "#596579", lineHeight: 1.65 }}>{selectedApproval.summary}</p>

              <div className="compact-list">
                <div><strong>Subject</strong><span>{selectedApproval.subject_type} · {selectedApproval.subject_id}</span></div>
                <div><strong>Requested</strong><span>{formatDate(selectedApproval.created_at)}</span></div>
                <div><strong>Expires</strong><span>{formatDate(selectedApproval.expires_at)}</span></div>
                <div><strong>Resolved</strong><span>{formatDate(selectedApproval.resolved_at)}</span></div>
              </div>

              <div style={{ marginTop: 18 }}>
                <p className="label">Conditions</p>
                {conditions.length ? (
                  <ul style={{ paddingLeft: 20, color: "#596579", lineHeight: 1.7 }}>
              ...
              {isPending ? (
                <form action={resolveApproval} className="auth-form" style={{ marginTop: 20 }}>
                  ...
                  <label>
                    ...
                    <textarea
                      ...
                      style={{ width: "100%", resize: "vertical", padding: 12, border: "1px solid #cfd6e2", borderRadius: 10, font: "inherit" }}
                    />
                  </label>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <button name="resolution" value="approved" type="submit">Approve request</button>
                    <button name="resolution" value="rejected" type="submit" style={{ background: "#8f2335" }}>Reject request</button>
                  </div>
                  <p className="security-note">Only the authenticated organization Owner can execute this resolution.</p>
                </form>
              ) : (
                <div style={{ marginTop: 18, padding: 14, borderRadius: 12, background: "#fff" }}>
                  <p className="label">CEO response</p>
                  <p style={{ color: "#596579", lineHeight: 1.6 }}>{selectedApproval.response_note ?? "No response note recorded."}</p>
                </div>
              )}
              ...
              <div style={{ marginTop: 22 }}>
                ...
                <div className="compact-list">
                  ...
                  )) : <p className="empty-state">No audit events recorded for this request.</p>}
                </div>
              </div>
            </article>
          ) : <p className="empty-state">Select an approval request to inspect its details.</p>}
        </div>
      </section>
    </main>
  );
