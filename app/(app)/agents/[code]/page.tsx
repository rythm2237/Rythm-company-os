...
async function requestLevelAssessment(formData:FormData){
  ...
  if(!membership) redirect("/login?error=Owner%20authorization%20required.");
  const agentId=String(formData.get("agentId")??"");
  const {data:agent}=await supabase.from("agents").select("id,agent_code,specification_version").eq("id",agentId).eq("organization_id",membership.organization_id).maybeSingle();
  if(!agent)return;
...
export default async function AgentProfile({params,searchParams}:{params:Promise<{code:string}>;searchParams:Promise<{error?:string;message?:string}>}){
  ...
  const {data:agent}=await supabase.from("agents").select("id, agent_code, display_name, name, role_title, purpose, department, avatar_url, presence_status, enabled, risk_ceiling, authority_level, work_style, supported_languages, identity, permissions, specification_version, agent_asset_profiles(current_level,level_score,certification_status,certification_version,certified_at,last_assessed_at,valuation_status,valuation_readiness_score,marketplace_eligible,canonical_name,identity_locked)").eq("organization_id",membership.organization_id).ilike("agent_code",code).maybeSingle();
  if(!agent)notFound();
