...
export default async function AgentKnowledgePage({params}:{params:Promise<{code:string}>}){
  ...
  if(!agent)notFound();
  const directAcl=`agent:${agent.id}`;
  const {data:knowledge}=await context.supabase.from("company_knowledge").select("id,title,category,source_filename,ingestion_status,chunk_count,updated_at").eq("organization_id",context.organizationId).contains("allowed_role_keywords",[directAcl]).order("updated_at",{ascending:false}).limit(50);
  ...
  return <main className="command-shell"><header className="command-header"><div><p className="eyebrow">AGENT KNOWLEDGE</p><h1>{name}</h1><p className="subtitle">Add role-specific knowledge without writing a prompt. Files stay tenant-private and are indexed through the governed Company Library pipeline.</p></div><Link className="secondary-button" href={`/agents/${String(agent.agent_code).toLowerCase()}`}>Back to Agent</Link></header>
    <section style={{display:"grid",gridTemplateColumns:"minmax(0,1.1fr) minmax(320px,.9fr)",gap:18,alignItems:"start"}}>
      <article className="panel"><p className="label">DIRECT KNOWLEDGE</p><h2>Files assigned only to this Agent</h2><p style={{color:"#667085",lineHeight:1.65}}>Company knowledge is inherited separately. Items listed here use an exact Agent ACL and do not become knowledge for sibling Agents.</p><div className="compact-list">{knowledge?.length?knowledge.map((item)=><div key={item.id}><strong>{item.title}</strong><span>{item.category} · {item.ingestion_status} · {item.chunk_count??0} chunks</span></div>):<p className="empty-state">No direct Agent knowledge has been added yet.</p>}</div></article>
      <article className="panel"><p className="label">ADD KNOWLEDGE</p><h2>Upload to {name}</h2><AgentKnowledgeUploader organizationId={context.organizationId} agentId={agent.id} agentCode={agent.agent_code} agentName={name}/></article>
    </section>
  </main>
}
