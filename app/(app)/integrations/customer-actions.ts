...
import { discoverGoogleResources, getTokenConnectionAdapter, googleProfile } from "@/lib/integrations/connections/providers";
...
export async function checkCustomerConnectionHealth(formData:FormData){
  ...
  let health:{healthy:boolean;code:string};if(adapter)health=await adapter.healthCheck(String(secret.data));else{try{const envelope=JSON.parse(String(secret.data)) as {access_token?:string};if(!envelope.access_token)throw new Error("missing");if(connection.data.provider_key==="google_search_console")await discoverGoogleResources("google_search_console",envelope.access_token);else if(connection.data.provider_key==="google_analytics")await discoverGoogleResources("google_analytics",envelope.access_token);else if(connection.data.provider_key==="google_workspace")await googleProfile(envelope.access_token);else if(connection.data.provider_key==="microsoft_365"){const response=await fetch("https://graph.microsoft.com/v1.0/me?$select=id",{headers:{Authorization:`Bearer ${envelope.access_token}`},cache:"no-store",signal:AbortSignal.timeout(20000)});if(!response.ok)throw Object.assign(new Error("Microsoft health check failed."),{status:response.status});}else throw new Error("unsupported");health={healthy:true,code:"verified"};}catch(error){health={healthy:false,code:(error as {status?:number}).status===401?"reauth_required":"provider_error"};}}
