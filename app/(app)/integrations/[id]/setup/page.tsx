...
async function saveCredential(formData:FormData){
  ...
  await context.supabase.from("organization_integrations").update({granted_scopes:grantedScopes,status:"disconnected",metadata:{setup_state:"credential_saved_unverified"},updated_at:new Date().toISOString()}).eq("id",integrationId).eq("organization_id",context.organizationId);
...
export default async function IntegrationSetupPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{message?:string;error?:string;project?:string}>}){
  ...
  return <main className="command-shell"><span hidden data-integration-guide-provider={integration.provider_key}/><header className="command-header"><div><p className="eyebrow">CONNECTION SETUP</p><h1>{help.title}</h1><p className="subtitle">{help.intro}</p></div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><IntegrationGuideButton providerKey={integration.provider_key} label="Keep guide open"/><Link className="secondary-button" href={backHref}>Back to connections</Link></div></header>
    ...
    <section className="panel" style={{maxWidth:760}}><div className="panel-heading"><div><p className="label">{provider?.display_name??integration.provider_key}</p><h2>{integration.display_name}</h2></div><span data-guide-target="verify" className={integration.status==="connected"?"state-active":"state-paused"}>{integration.status==="connected"?"Connected":"Setup required"}</span></div>
      {integration.status==="connected"?<p>This connection has been verified. Return to the project to choose the exact repository, property, site, account or workspace and grant only the capabilities that project needs.</p>:oauth?<div data-guide-target="credential"><p>{oauth.copy}</p><form action={oauth.action} method="post" target="_blank" rel="opener" style={{marginTop:16}}><input type="hidden" name="integrationId" value={integration.id}/>{integration.provider_key==="google_workspace"?<input type="hidden" name="displayName" value={integration.display_name}/>:null}<button data-guide-target="start-setup" className="primary-button" type="submit">{oauth.button}</button></form><p className="integration-security-note">The external provider must complete authorization successfully before RYTHM changes this status to <strong>Connected</strong>. A local connection record alone is not a successful integration.</p></div>:useToken?<form action={saveCredential} className="stacked-form"><input type="hidden" name="integrationId" value={integration.id}/><input type="hidden" name="grantedScopes" value={help.scopes}/><label>{help.credentialLabel}<input data-guide-target="credential" type="password" name="secret" autoComplete="off" required placeholder="Paste the provider-issued token"/></label><p className="integration-security-note">{help.credentialHelp}</p><button data-guide-target="start-setup" className="primary-button" type="submit">Save securely & continue</button></form>:<div data-guide-target="credential"><p>This service requires provider authorization, but its customer OAuth adapter is not enabled in this RYTHM release yet.</p><p className="integration-security-note">The service remains <strong>Setup required</strong>. RYTHM will not accept a generic token or display a successful connection message until a real provider-specific authorization flow exists.</p></div>}
