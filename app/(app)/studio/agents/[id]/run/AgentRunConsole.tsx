...
import { runAgentConsole, type ChartSpec, type OutputPreference } from "./actions";
...
function RichText({ content }: { content: string }) {
  ...
  return (
    <div style={{ display: "grid", gap: ".42rem", lineHeight: 1.65 }}>
      {lines.map((raw, index) => {
...
function ChartArtifact({ spec }: { spec: ChartSpec }) {
  ...
  const baseline = y(0);
  const linePoints = spec.points.map((point, index) => `${x(index)},${y(point.value)}`).join(" ");
  const barSlot = plotWidth / spec.points.length;
  const barWidth = Math.max(10, Math.min(52, barSlot * .58));

  return (
    <div style={{ border: "1px solid rgba(110,120,140,.22)", borderRadius: 16, padding: "1rem", background: "rgba(255,255,255,.72)", overflowX: "auto" }}>
      <div style={{ minWidth: 640 }}>
        ...
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={spec.title} style={{ width: "100%", height: "auto", display: "block" }}>
          {[0, .25, .5, .75, 1].map((ratio) => {
            ...
            return (
              <Fragment key={ratio}>
                <line x1={left} x2={width - right} y1={yy} y2={yy} stroke="currentColor" opacity="0.09" />
                <text x={left - 10} y={yy + 4} textAnchor="end" fontSize="12" fill="currentColor" opacity="0.6">{Number(value.toFixed(1)).toLocaleString()}</text>
              </Fragment>
            );
          })}
          ...
          {spec.type === "line" ? (
            <>
              <polyline points={linePoints} fill="none" stroke="currentColor" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" />
              {spec.points.map((point, index) => <circle key={point.label} cx={x(index)} cy={y(point.value)} r="5" fill="currentColor" />)}
            </>
          ) : (
            spec.points.map((point, index) => {
              const yy = y(Math.max(point.value, 0));
              const h = Math.abs(y(point.value) - baseline);
              return <rect key={point.label} x={left + index * barSlot + (barSlot - barWidth) / 2} y={point.value >= 0 ? yy : baseline} width={barWidth} height={Math.max(2, h)} rx="5" fill="currentColor" opacity="0.82" />;
            })
          )}
          {spec.points.map((point, index) => {
            const every = spec.points.length > 12 ? 2 : 1;
            if (index % every !== 0 && index !== spec.points.length - 1) return null;
            const xx = spec.type === "bar" ? left + index * barSlot + barSlot / 2 : x(index);
            return <text key={`${point.label}-label`} x={xx} y={height - 38} textAnchor="middle" fontSize="11" fill="currentColor" opacity="0.68">{point.label.slice(0, 14)}</text>;
          })}
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await runAgentConsole({
        ...
        prompt: value,
        mode,
        outputPreference: mode === "chat" ? "text" : outputPreference,
        messages: transcript,
      });
    ...
    <div style={{ display: "grid", gap: "1rem" }}>
      <section className="panel" style={{ margin: 0 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: "1rem", flexWrap: "wrap" }}>
          <div>
            <h2 style={{ marginBottom: ".2rem" }}>{agentName}</h2>
            <p style={{ margin: 0 }}>{roleTitle}</p>
          </div>
          <div style={{ textAlign: "right", fontSize: ".88rem", opacity: .78 }}>
            <div>Status: <strong>{status}</strong></div>
            <div>Brain: <strong>{provider}</strong> · {model}</div>
          </div>
        </div>
        <p style={{ marginBottom: 0 }}>
          Talk naturally. In Task mode, leave output on <strong>Auto</strong> and RYTHM will choose the best format, or select a format yourself when you already know what you want.
        </p>
      </section>
      ...
      <section className="panel" style={{ margin: 0 }}>
        ...
        {messages.length === 0 ? (
          <div style={{ padding: ".75rem 0 1.15rem" }}>
            <h3>Work with {agentName}</h3>
            <p>Ask a question, request advice, or give a concrete assignment. The output can change from one message to the next.</p>
          </div>
        ) : (
          <div aria-live="polite" style={{ display: "grid", gap: ".95rem", marginBottom: "1.25rem" }}>
            {messages.map((message) => (
              <article key={message.id} className="kpi-card" style={{ maxWidth: "100%", overflow: "hidden" }}>
                <p className="eyebrow">{message.role === "user" ? "YOU" : agentName.toUpperCase()}</p>
                {message.responseType === "image" && message.imageDataUrl ? (
                  <div style={{ display: "grid", gap: ".75rem" }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={message.imageDataUrl} alt={`${agentName} generated visual`} style={{ width: "100%", maxHeight: 720, objectFit: "contain", borderRadius: 14, background: "rgba(0,0,0,.035)" }} />
                    <RichText content={message.content} />
                  </div>
                ) : message.responseType === "chart" && message.chartSpec ? (
                  <div style={{ display: "grid", gap: ".75rem" }}>
                    <ChartArtifact spec={message.chartSpec} />
                    {message.content && message.content !== message.chartSpec.insight ? <RichText content={message.content} /> : null}
                  </div>
                ) : (
                  <RichText content={message.content} />
                )}
        ...
        <label style={{ display: "grid", gap: ".5rem" }}>
          ...
          <textarea
            ...
            placeholder={mode === "task" ? "Describe the result you want. You can leave the output format on Auto…" : "Ask a question or discuss an idea…"}
            maxLength={12000}
          />
        </label>
        ...
        {mode === "task" ? (
          <div style={{ marginTop: ".85rem" }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: ".75rem", flexWrap: "wrap" }}>
              <strong style={{ fontSize: ".88rem" }}>Output</strong>
              <span style={{ opacity: .62, fontSize: ".78rem" }}>Optional — Auto is recommended</span>
            </div>
            <div style={{ display: "flex", gap: ".55rem", flexWrap: "wrap", marginTop: ".5rem" }}>
              {options.map((option) => (
                <button
                  ...
                  aria-pressed={outputPreference === option.value}
                  title={option.hint}
                  onClick={() => !option.disabled && setOutputPreference(option.value as OutputPreference)}
                  style={{ opacity: option.disabled ? .45 : 1 }}
                >
                  {outputPreference === option.value ? "✓ " : ""}{option.label}{option.disabled ? " · Soon" : ""}
                </button>
              ))}
            </div>
            <p style={{ margin: ".45rem 0 0", opacity: .62, fontSize: ".78rem" }}>
              {options.find((option) => option.value === outputPreference)?.hint ?? "RYTHM will choose the best output."}
            </p>
          </div>
        ) : null}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: ".75rem", flexWrap: "wrap", marginTop: ".9rem" }}>
          ...
          <span style={{ opacity: .62, fontSize: ".8rem" }}>This preview remains read-only for external actions.</span>
        </div>
      </section>
    </div>
  );
