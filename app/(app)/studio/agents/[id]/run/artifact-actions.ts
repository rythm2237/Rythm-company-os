...
function safeFilename(value: string) {
  return cleanTitle(value).toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "agent-artifact";
}
...
export async function saveAgentArtifact(input: SaveArtifactInput) {
  ...
  try {
    ...
    const { data: agent, error: agentError } = await context.supabase.from("agents").select("id").eq("id", input.agentId).eq("organization_id", context.organizationId).maybeSingle();
    ...
    const { data, error } = await context.supabase.from("agent_artifacts").insert({
      ...
      structured_content: input.chartSpec ?? null,
      storage_path: storagePath,
      mime_type: mimeType,
      metadata: input.metadata ?? {},
      source_company_id: context.organizationId,
      transferable: false,
    ...
    if (uploadedPath) {
      try {
        const context = await requireActiveOwnerOrganizationContext();
        await context.supabase.storage.from("agent-artifacts").remove([uploadedPath]);
      } catch {}
    }
