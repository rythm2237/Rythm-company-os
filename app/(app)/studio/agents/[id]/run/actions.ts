...
import { planContext, renderContextPlan } from "@/lib/ai/context-planner";
import type { AgentRoutingPolicy, CostStrategy, ModelPolicy, ModelTier, RiskLevel, RoutingDecision, TenantAiPolicy } from "@/lib/ai/routing-types";
...
type AgentRuntimeRow = {
  ...
  model_policy: Partial<ModelPolicy> | null;
  language_policy: Record<string, unknown> | null;
...
type ActiveContext = Awaited<ReturnType<typeof requireActiveOwnerOrganizationContext>>;
type RuntimeRouting = {
  agentPolicy: AgentRoutingPolicy;
  tenantPolicy: TenantAiPolicy;
  onRoutingDecision: (decision: RoutingDecision) => Promise<void>;
...
const AGENT_RUNTIME_FIELDS = "id,name,role_title,department,agent_status,provisioning_status,external_actions_allowed,system_instructions,runtime_provider,runtime_model,model_policy,language_policy,routing_policy_key,allowed_tools,risk_ceiling,language,authority_level";

function safeMessage(error: unknown) {
  const message = error instanceof Error ? error.message : "Agent execution failed.";
  if (/not configured|empty Agent response|request failed|timed out|timeout|not available|image generation|file|attachment|provider|knowledge|credit|quota|billing|429|provision|budget|restricted handling/i.test(message)) return message;
...
function validRisk(value: string | null): RiskLevel | undefined { return value === "low" || value === "medium" || value === "high" || value === "restricted" ? value : undefined; }
function validTiers(value: unknown): ModelTier[] {
  if (!Array.isArray(value)) return ["luna", "terra", "sol"];
  const result = value.filter((item): item is ModelTier => item === "luna" || item === "terra" || item === "sol");
  return result.length ? result : ["luna", "terra", "sol"];
}
function validCostStrategy(value: unknown): CostStrategy { return value === "economy" || value === "quality" ? value : "balanced"; }
...
function extractJsonObject(value: string) { const fenced = value.match(/```(?:json)?\s*([\s\S]*?)```/i)?.[1]; const start = value.indexOf("{"), end = value.lastIndexOf("}"); return (fenced ?? (start >= 0 && end > start ? value.slice(start, end + 1) : value)).trim(); }
...
async function getAgent(agentId: string) {
  ...
  const { data, error } = await context.supabase.from("agents").select(AGENT_RUNTIME_FIELDS).eq("id", agentId).eq("organization_id", context.organizationId).maybeSingle();
  if (error || !data) throw new Error("Agent not found in this organization.");
  return { context, agent: data as unknown as AgentRuntimeRow };
}

async function createRoutingRuntime(context: ActiveContext, agent: AgentRuntimeRow): Promise<RuntimeRouting> {
  const { data: userPreference } = await context.supabase.from("user_ai_preferences").select("preferred_language").eq("user_id", context.user.id).maybeSingle();
  const tenantLimit = context.entitlement.max_ai_cost_per_request == null ? undefined : Number(context.entitlement.max_ai_cost_per_request);
  const rawModelPolicy = agent.model_policy ?? {};
  const configuredLimit = rawModelPolicy.maxCostPerRequest == null ? undefined : Number(rawModelPolicy.maxCostPerRequest);
  const maxCostPerRequest = tenantLimit == null ? configuredLimit : configuredLimit == null ? tenantLimit : Math.min(tenantLimit, configuredLimit);
  const agentPolicy: AgentRoutingPolicy = {
    agentId: agent.id,
    roleTitle: agent.role_title,
    allowedTools: Array.isArray(agent.allowed_tools) ? agent.allowed_tools : [],
    riskCeiling: validRisk(agent.risk_ceiling),
    modelPolicy: { ...rawModelPolicy, ...(maxCostPerRequest == null ? {} : { maxCostPerRequest }) },
    savedLanguage: userPreference?.preferred_language ?? agent.language ?? null,
  };
  const tenantPolicy: TenantAiPolicy = {
    allowedTiers: validTiers(context.entitlement.allowed_model_tiers),
    aiBudgetLimit: Number(context.entitlement.ai_budget_limit ?? 0),
    costStrategy: validCostStrategy(context.entitlement.preferred_cost_strategy),
    maxContextTokens: context.entitlement.max_ai_context_tokens,
    advancedReasoningAllowed: context.entitlement.advanced_reasoning_enabled,
  };
  let lastDecision: RoutingDecision | null = null;
  const onRoutingDecision = async (decision: RoutingDecision) => {
    lastDecision = decision;
    const { error } = await context.supabase.from("ai_routing_decisions").upsert({
      ...
      response_language: decision.responseLanguage,
      intent: decision.intent,
      intent_taxonomy_version: decision.intentTaxonomyVersion,
      complexity: decision.complexity,
      risk_level: decision.risk,
      selected_model_tier: decision.selectedTier,
      proposed_model_tier: decision.selectedTier,
      proposed_capability_tier: decision.selectedCapabilityTier,
      provider: decision.selectedProvider,
      provider_model: decision.selectedModel,
      proposed_provider: decision.selectedProvider,
      proposed_model: decision.selectedModel,
      reasoning_level: decision.reasoningLevel,
      reasoning_depth: decision.reasoningDepth,
      routing_confidence: decision.confidence,
      routing_source: decision.classificationSource,
      escalation_index: decision.escalationIndex,
      tools_used: decision.requiredTools,
      reason_codes: decision.reasonCodes,
      estimated_cost_usd: decision.estimatedCostUsd,
      estimated_latency_ms: decision.estimatedLatencyMs,
      authorization_signal: decision.authorizationSignal,
      human_review_required: decision.humanReviewRequired,
      context_requirements: decision.contextRequirements,
      required_modalities: decision.requiredModalities,
      router_version: decision.routingVersion,
      adaptive_policy_version: decision.policyVersion,
      classifier_version: decision.classifierVersion,
      model_registry_version: decision.modelRegistryVersion,
      execution_status: decision.escalationIndex > 0 ? "escalated" : "routed",
      validation_result: decision.routingVersion,
    }, { onConflict: "request_id,escalation_index" });
    if (error) console.warn("[RYTHM AI Telemetry] routing decision persistence failed", { requestId: decision.requestId, errorCode: error.code });
  };
  return { agentPolicy, tenantPolicy, onRoutingDecision, getLastDecision: () => lastDecision };
}

function buildContext(prompt: string, companyContext: string, memory: string, history: string, runtime: RuntimeRouting, maxContextTokens: number | null) {
  const intelligence = deterministicRequestIntelligence({ prompt, savedLanguage: runtime.agentPolicy.savedLanguage, agent: runtime.agentPolicy });
  const maxCharacters = maxContextTokens ? Math.min(120000, Math.max(4000, maxContextTokens * 4)) : undefined;
  return renderContextPlan(planContext({
    intelligence,
    maxCharacters,
    layers: [
      { kind: "tenant_context", content: companyContext, priority: 100 },
      { kind: "memory", content: memory, priority: 70 },
      { kind: "conversation", content: history, priority: 60 },
    ],
  }));
}
...
async function loadAttachments(context: ActiveContext, agentId: string, ids: string[]) {
  ...
  for (const row of (data ?? []) as AttachmentRow[]) { const { data: blob, error: downloadError } = await context.supabase.storage.from("agent-attachments").download(row.storage_path); if (downloadError || !blob) throw new Error(`Could not read ${row.filename}.`); const buffer = Buffer.from(await blob.arrayBuffer()); files.push({ filename: row.filename, mimeType: row.mime_type, base64: buffer.toString("base64") }); }
  return files;
}
async function loadMemoryContext(context: ActiveContext, agentId: string) {
...
async function rememberExperience(context: ActiveContext, agentId: string, prompt: string, response: string, outputType: string, attachmentIds: string[]) {
...
async function generateVisualWithQa(input: { context: ActiveContext; agent: AgentRuntimeRow; style: "image" | "mockup"; brief: string; companyContext: string; roleQaRules: string[]; references: AgentAttachmentInput[] }) {
...
async function generateChart(runtimeArgs: Omit<Parameters<typeof runAgent>[0], "prompt" | "conversation" | "attachments" | "mode">, agent: AgentRuntimeRow, prompt: string, conversation: string, chartType: "line" | "bar", attachments: AgentAttachmentInput[]) {
  ...
  const raw = await runAgentWithAttachmentFallback({ ...runtimeArgs, prompt: chartPrompt, attachments, mode: "task" });
...
export async function runAgentConsole(input: RunConsoleInput) {
  ...
  try {
    ...
    const provider = (agent.runtime_provider ?? "openai") as AgentProvider; if (!(["openai", "anthropic", "google"] as string[]).includes(provider)) return { ok: false as const, error: "This Agent uses an unsupported fallback runtime provider." };
    ...
    const runtime = await createRoutingRuntime(context, agent);
    const conversationContext = buildContext(prompt, companyKnowledge.contextText, memory, history, runtime, context.entitlement.max_ai_context_tokens);
    const runtimeArgs = {
      provider,
    ...
    if (resolvedOutput === "line-chart" || resolvedOutput === "bar-chart") {
      const chart = await generateChart(runtimeArgs, agent, prompt, conversationContext, resolvedOutput === "line-chart" ? "line" : "bar", attachments);
      const decision = runtime.getLastDecision(); const effectiveProvider = decision?.selectedProvider ?? provider; const effectiveModel = decision?.selectedModel ?? agent.runtime_model;
      if (chart.needsData) { await rememberExperience(context, agent.id, prompt, chart.message, resolvedOutput, attachmentIds); return { ok: true as const, responseType: "text" as const, response: chart.message, resolvedOutput, provider: effectiveProvider, model: effectiveModel, latencyMs: Date.now() - started, agentName: agent.name, roleTitle: agent.role_title, status: agent.agent_status, externalActions: false, knowledgeCount: companyKnowledge.knowledgeCount, professionalFoundation: companyKnowledge.professionalFoundation }; }
      const responseText = chart.spec.insight || "Chart generated from the supplied data."; await rememberExperience(context, agent.id, prompt, responseText, resolvedOutput, attachmentIds);
      return { ok: true as const, responseType: "chart" as const, response: responseText, chartSpec: chart.spec, resolvedOutput, provider: effectiveProvider, model: effectiveModel, latencyMs: Date.now() - started, agentName: agent.name, roleTitle: agent.role_title, status: agent.agent_status, externalActions: false, knowledgeCount: companyKnowledge.knowledgeCount, professionalFoundation: companyKnowledge.professionalFoundation };
    }
    ...
    const response = await runAgentWithAttachmentFallback({ ...runtimeArgs, prompt: `${chatGuard}${reportGuard}${prompt}`, conversation: conversationContext, attachments, mode: input.mode === "task" ? "task" : "chat" });
    const decision = runtime.getLastDecision(); const effectiveProvider = decision?.selectedProvider ?? provider; const effectiveModel = decision?.selectedModel ?? agent.runtime_model;
