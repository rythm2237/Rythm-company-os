...
function safeError(error: unknown) {
  ...
  if (/not active|not enabled|limit reached|owner authority|invalid|cannot|not found|not configured|empty Agent instruction|request failed|incomplete|foundation|knowledge|specialization|coverage|provision|trusted source/i.test(message)) return message;
...
async function resolveWithSpecializationAcquisition(context: ProvisioningContext, agentId: string, roleTitle: string, provider: AgentProvider, model: string) {
  ...
  try {
    ...
    await acquireMissingSpecializations({ supabase: context.supabase, organizationId: context.organizationId, agentId, normalized, provider, model });
...
async function resolveOrAcquireKnowledge(context: ProvisioningContext, agentId: string, roleTitle: string, provider: AgentProvider, model: string) {
  const normalized = normalizeRole(roleTitle);
  let knowledge = await resolveWithSpecializationAcquisition(context, agentId, roleTitle, provider, model);
  if (knowledge.fallbackUsed && normalized.roleFamily !== "general") {
    await context.supabase.from("agent_knowledge_provisioning_events").insert({
      organization_id: context.organizationId, agent_id: agentId, event_type: "acquisition_triggered",
      role_family: normalized.roleFamily, canonical_role: normalized.canonicalRole, metadata: { reason: "missing_active_foundation" },
    });
    const acquiredId = await acquireMissingFoundation({ supabase: context.supabase, normalized, provider, model });
    if (acquiredId) knowledge = await resolveWithSpecializationAcquisition(context, agentId, roleTitle, provider, model);
...
async function ensureKnowledgeBindings(context: ProvisioningContext, agentId: string, knowledge: ResolvedKnowledgePackage) {
  const { data: activeFoundation } = await context.supabase.from("agent_role_foundation_bindings").select("id,role_foundation_id")
    .eq("organization_id", context.organizationId).eq("agent_id", agentId).eq("status", "active").limit(1).maybeSingle();
  if (!activeFoundation) { await bindKnowledgePackage(context.supabase, context.organizationId, agentId, knowledge); return; }
  if (activeFoundation.role_foundation_id !== knowledge.foundation.id) {
    const { error: supersedeError } = await context.supabase.from("agent_role_foundation_bindings").update({ status: "superseded" })
      .eq("id", activeFoundation.id).eq("organization_id", context.organizationId);
    if (supersedeError) throw new Error(`Could not supersede old professional foundation: ${supersedeError.message}`);
    await context.supabase.from("agent_specialization_bindings").update({ status: "detached" })
      .eq("organization_id", context.organizationId).eq("agent_id", agentId).eq("status", "active");
    await bindKnowledgePackage(context.supabase, context.organizationId, agentId, knowledge);
    return;
  }
  const { data: existingSpecs } = await context.supabase.from("agent_specialization_bindings").select("specialization_id")
    .eq("organization_id", context.organizationId).eq("agent_id", agentId).eq("status", "active");
  ...
  await context.supabase.from("agents").update({ foundation_update_available: knowledge.updateAvailable })
...
async function finalizeProfessionalProvisioning(context: ProvisioningContext, agentId: string, input: Parameters<typeof buildAgentBlueprint>[0], provider: AgentProvider, model: string) {
  ...
  await context.supabase.from("agent_knowledge_provisioning_events").insert({
    organization_id: context.organizationId, agent_id: agentId, event_type: "company_knowledge_connected",
  ...
  await context.supabase.from("agents").update({
    raw_role_title: knowledge.normalized.rawRoleTitle, canonical_role: knowledge.normalized.canonicalRole,
    role_family: knowledge.normalized.roleFamily, specializations: knowledge.normalized.specializations,
    company_knowledge_connected: true, company_knowledge_detached_at: null,
  }).eq("organization_id", context.organizationId).eq("id", agentId);
...
export async function generateAgent(formData: FormData) {
  ...
  const { data: createdAgentId, error: createError } = await context.supabase.rpc("create_agent_provisioning_v3", {
    target_org_id: context.organizationId, target_name: input.name, target_role_title: input.roleTitle, target_purpose: input.purpose,
    target_runtime_provider: provider, target_runtime_model: providerOption.model, target_raw_role_title: normalized.rawRoleTitle,
    target_canonical_role: normalized.canonicalRole, target_role_family: normalized.roleFamily, target_specializations: normalized.specializations,
    ...
    target_authority_level: input.authorityLevel, target_risk_ceiling: input.riskCeiling, target_language: input.language,
    target_work_style: input.workStyle, target_responsibilities: input.responsibilities, target_skills: input.skills, target_kpis: input.kpis,
    target_human_approval_requirements: input.approvalRequirements, target_allowed_tools: input.allowedTools,
  });
  ...
  try { await finalizeProfessionalProvisioning(context, agentId, input, provider, providerOption.model); }
  catch (error) {
...
export async function retryAgentProvisioning(formData: FormData) {
  ...
  const retryInput = {
    ...
    responsibilities: agent.responsibilities ?? [], skills: agent.skills ?? [], kpis: agent.kpis ?? [], language: agent.language ?? "English",
    workStyle: agent.work_style ?? "Evidence-led and concise", authorityLevel: Number(agent.authority_level ?? 1), riskCeiling: String(agent.risk_ceiling ?? "medium"),
    approvalRequirements: agent.human_approval_requirements ?? [], allowedTools: agent.allowed_tools ?? [],
  };
  await context.supabase.from("agents").update({ provisioning_status: "provisioning", provisioning_error: null, provisioning_started_at: new Date().toISOString() })
    .eq("organization_id", context.organizationId).eq("id", agentId);
  try { await finalizeProfessionalProvisioning(context, agentId, retryInput, provider, model); }
...
export async function updateAgent(formData: FormData) {
  ...
  const { error } = await context.supabase.rpc("update_agent_v1", {
    ...
    target_reports_to_agent_id: String(formData.get("reportsToAgentId") ?? "") || null, target_authority_level: Number(formData.get("authorityLevel") ?? 1),
    target_risk_ceiling: String(formData.get("riskCeiling") ?? "medium"), target_language: String(formData.get("language") ?? "English"),
    target_responsibilities: list(formData.get("responsibilities")), target_skills: list(formData.get("skills")), target_kpis: list(formData.get("kpis")),
    target_human_approval_requirements: list(formData.get("approvalRequirements")), target_allowed_tools: list(formData.get("allowedTools")),
  });
  if (error) redirect(`/studio/agents/${agentId}?error=${encodeURIComponent(safeError(error))}`);
  const oldNormalized = normalizeRole(String(currentAgent?.role_title ?? ""));
  const newNormalized = normalizeRole(newRoleTitle);
  const roleChanged = oldNormalized.canonicalRole !== newNormalized.canonicalRole || oldNormalized.roleFamily !== newNormalized.roleFamily || oldNormalized.specializations.join("|") !== newNormalized.specializations.join("|");
  if (roleChanged && String(currentAgent?.template_version ?? "").startsWith("trusted-bootstrap")) {
    await context.supabase.from("agent_role_foundation_bindings").update({ status: "superseded" }).eq("organization_id", context.organizationId).eq("agent_id", agentId).eq("status", "active");
    ...
    await context.supabase.from("agents").update({
      raw_role_title: newNormalized.rawRoleTitle, canonical_role: newNormalized.canonicalRole, role_family: newNormalized.roleFamily,
      specializations: newNormalized.specializations, provisioning_status: "failed",
      provisioning_error: "Role changed. Professional knowledge must be re-provisioned before this Agent can run.",
      agent_status: "paused", enabled: false, foundation_update_available: true,
    }).eq("organization_id", context.organizationId).eq("id", agentId);
    await context.supabase.from("agent_knowledge_provisioning_events").insert({
      organization_id: context.organizationId, agent_id: agentId, event_type: "role_changed_reprovision_required",
      role_family: newNormalized.roleFamily, canonical_role: newNormalized.canonicalRole,
      metadata: { previous_role_family: oldNormalized.roleFamily, previous_canonical_role: oldNormalized.canonicalRole },
    });
  }
  revalidatePath(`/studio/agents/${agentId}`); revalidatePath("/studio/agents"); revalidatePath("/agents");
  redirect(`/studio/agents/${agentId}?message=${encodeURIComponent(roleChanged ? "Agent profile updated. Role changed, so professional re-provisioning is required." : "Agent profile updated.")}`);
}
