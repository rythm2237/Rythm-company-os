...
type CompanyTemplateRow = {
  ...
  catalog_slug: string | null; display_order: number; industry_tags: string[]; recommended_for: string[];
  maturity: string; is_featured: boolean; compatibility_contract: Record<string, unknown>; upgrade_strategy: string;
};
type AgentTemplateRow = {
  ...
  monthly_company_cost: number; cost_currency: string; cost_model: string; sale_price_monthly: number | null;
};
type WorkflowRow = {
...
type InstallationRow = { template_key: string; template_version: string; installed_at: string; template_snapshot_digest: string | null; upgrade_status: string };
...
export default async function CompanyTemplateLibraryPage({ searchParams }: TemplatePageProps) {
  ...
  if (!context.entitlement.company_template_access) {
    return <main className="page-shell"><section className="panel"><p className="eyebrow">RYTHM READY COMPANY LIBRARY</p><h1>Ready AI Companies</h1><p>Company Template access is not enabled for this organization.</p><Link href="/command-center">Return to Command Center</Link></section></main>;
  }
  ...
  const [templatesResult, agentsResult, installationsResult, workflowsResult, meetingsResult, requirementsResult, dependenciesResult, tenantAgentsResult] = await Promise.all([
    context.supabase.from("company_templates").select("id,template_key,name,company_type,category,description,positioning,version,supported_product_codes,department_templates,agent_template_refs,workflow_template_refs,governance_profile,launch_configuration,catalog_slug,display_order,industry_tags,recommended_for,maturity,is_featured,compatibility_contract,upgrade_strategy").eq("status", "active").order("display_order").order("name"),
    context.supabase.from("agent_templates").select("id,template_key,version,name,role,purpose,role_family,default_specializations,default_risk_ceiling,monthly_company_cost,cost_currency,cost_model,sale_price_monthly").eq("is_active", true).order("name"),
    context.supabase.from("organization_template_installations").select("template_key,template_version,installed_at,template_snapshot_digest,upgrade_status").eq("organization_id", context.organizationId),
    context.supabase.from("company_template_workflows").select("workflow_key,version,company_template_key,name,description,stages").eq("active", true),
  ...
  return <main className="page-shell">
    <section className="panel">
      <p className="eyebrow">RYTHM READY COMPANY LIBRARY · PHASE 4</p>
      <h1>Choose a governed AI company operating system</h1>
      <p>Each version is an immutable catalog release. Provisioning creates tenant-owned departments and paused AI Agents; company memory, credentials and external execution authority are never copied from the catalog.</p>
    ...
    {templates.length ? templates.map((template) => {
      const installation = installations.find((row) => row.template_key === template.template_key && row.template_version === template.version);
      ...
      const hasCapacity = context.entitlement.max_active_agents >= template.agent_template_refs.length;
      ...
      const canStartLegacyBlueprint = installed && template.template_key === "ready_software_company_v1";

      return <section className="panel panel-wide" key={`${template.template_key}-${template.version}`}>
        <div className="panel-heading"><div><p className="eyebrow">{template.is_featured ? "FEATURED · " : ""}{template.category}</p><h2>{template.name}</h2></div><span className="pill">{installed ? "Installed" : `v${template.version} · ${template.maturity}`}</span></div>
        <p>{template.description}</p>{template.positioning ? <p>{template.positioning}</p> : null}
        {template.industry_tags.length ? <p className="label">{template.industry_tags.join(" · ")}</p> : null}
        <div className="compact-list">
          ...
          <div><strong>{template.agent_template_refs.length} professional AI Agents</strong><span>Tenant-owned and paused at launch. External actions remain disabled.</span></div>
          <div><strong>Human CEO governance</strong><span>High-risk actions remain behind the existing approval and execution gateway.</span></div>
          <div><strong>Version snapshot isolation</strong><span>{installation?.template_snapshot_digest ? `Installed snapshot ${installation.template_snapshot_digest.slice(0, 12)}…` : "The exact catalog release is captured at installation."}</span></div>
          {template.recommended_for.length ? <div><strong>Recommended for</strong><span>{template.recommended_for.join(" · ")}</span></div> : null}
        </div>
        ...
        {!installed ? <form action={provisionCompanyTemplate} style={{ marginTop: 18 }}>
          ...
          <button className="primary-button" type="submit" disabled={!supported || !hasCapacity}>Provision this company</button>
          {!supported ? <p className="form-error">This template is not included in the active product entitlement.</p> : null}
          {!hasCapacity ? <p className="form-error">This template requires capacity for {template.agent_template_refs.length} active Agents; this organization allows {context.entitlement.max_active_agents}.</p> : null}
        </form> : canStartLegacyBlueprint ? <form action={startSoftwareProjectBlueprint} className="stacked-form" style={{ maxWidth: 620, marginTop: 18 }}>
        ...
        </form> : <div style={{ marginTop: 18 }}><span className="pill">Snapshot locked · {installation?.upgrade_status ?? "current"}</span> <Link href="/integrations">Configure tenant-owned integrations</Link></div>}
        ...
        <div className="executive-grid" style={{ marginTop: 24 }}>
          ...
          <article><p className="label">SETUP DEPENDENCIES</p><div className="compact-list">{templateRequirements.map((requirement) => { const dependency = templateDependencies.find((item) => item.dependency_key === requirement.provider_key); return <div key={requirement.provider_key}><strong>{requirement.provider_key} · {dependency?.status ?? (installed ? "pending" : requirement.requirement_level)}</strong><span>{requirement.purpose}</span></div>; })}</div></article>
        </div>
      </section>;
    ...
    <section className="panel panel-wide">
      ...
      <p>Individual role provisioning remains available to entitled Company Studio organizations. It creates a paused tenant instance and never copies company memory, credentials or external authority.</p>
      <div className="data-list">{agentCatalog.map((agent) => { const alreadyPresent = tenantAgentTemplateIds.has(agent.id); return <div className="data-row" key={agent.id}><div><strong>{agent.name}</strong><span>{agent.role_family} · {agent.default_specializations.join(", ") || "foundation"} · risk ceiling {agent.default_risk_ceiling}</span><span>{agent.purpose}</span><span>Company cost: {money(Number(agent.monthly_company_cost), agent.cost_currency)} ({agent.cost_model}) · Sale price: {money(agent.sale_price_monthly == null ? null : Number(agent.sale_price_monthly), agent.cost_currency)}</span></div><form action={provisionAgentTemplate}><input type="hidden" name="templateKey" value={agent.template_key}/><input type="hidden" name="templateVersion" value={agent.version}/><button className="secondary-button" disabled={alreadyPresent}>{alreadyPresent ? "Provisioned" : "Add Agent"}</button></form></div>; })}</div>
    </section>
