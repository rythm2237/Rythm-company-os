...
export default async function AdminSeoPage({ searchParams }: Props) {
  ...
  return <main className="admin-studio">
    ...
    <section className="admin-panel"><div className="admin-panel-heading"><div><p className="admin-kicker">HEALTH TREND</p><h2>Technical score history</h2></div><span>{scores.length} snapshots</span></div>
      {scores.length < 3 ? <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:12, marginTop:12 }}>{scores.map((item,index) => <article key={`${item.created_at}-${index}`} style={{ padding:16, border:"1px solid color-mix(in srgb,currentColor 12%,transparent)", borderRadius:14 }}><small style={{opacity:.65}}>{index === scores.length-1 ? "Latest" : `Snapshot ${index+1}`}</small><div style={{fontSize:28,fontWeight:800,marginTop:4}}>{item.score}/100</div><small style={{opacity:.65}}>{new Date(item.created_at).toLocaleString("en-GB",{timeZone:"Europe/Budapest"})}</small></article>)}</div> : <><div style={{ display:"flex", alignItems:"end", gap:8, height:150, paddingTop:12 }}>{scores.map((item,index) => <div key={`${item.created_at}-${index}`} title={`${item.score}/100 · ${new Date(item.created_at).toLocaleString("en-GB",{timeZone:"Europe/Budapest"})}`} style={{ flex:1, minWidth:12, height:`${Math.max(4,item.score)}%`, background:"currentColor", opacity:.18 + index / Math.max(1,scores.length)*.55, borderRadius:"6px 6px 2px 2px" }} />)}</div><div style={{display:"flex",justifyContent:"space-between",fontSize:11,opacity:.6}}><span>Older</span><span>Latest</span></div></>}
      {!scores.length ? <p className="admin-empty">No technical history has been recorded yet.</p> : scores.length < 3 ? <p style={{marginTop:12,opacity:.7,fontSize:13}}>A trend chart will appear after at least three monitoring snapshots.</p> : null}
    </section>
