...
export default async function AgentDirectory(){
  ...
  return <main className="command-shell">
    ...
    <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:18,marginTop:22}}>
      {agents.map(agent=>{const src=avatarSrc(agent.avatar_url);return <Link key={agent.id} href={`/agents/${agent.agent_code.toLowerCase()}`} style={{textDecoration:"none",color:"inherit",border:"1px solid #dfe4ec",borderRadius:20,overflow:"hidden",background:"#fff",boxShadow:"0 12px 30px rgba(25,35,58,.07)"}}>
        <div style={{height:230,background:"#e9edf4",position:"relative",display:"grid",placeItems:"center"}}>{src?<img src={src} alt={`${agent.display_name??agent.name} portrait`} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{fontSize:56,fontWeight:800}}>{agent.agent_code}</div>}<span style={{position:"absolute",right:14,top:14,background:"rgba(255,255,255,.9)",borderRadius:999,padding:"7px 11px",fontSize:12,fontWeight:700}}>{agent.presence_status.replace("_"," ")}</span></div>
