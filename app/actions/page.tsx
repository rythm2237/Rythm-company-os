...
type ActionRow = {
  id:string; project_id:string|null; meeting_id:string|null; decision_id:string|null;
  action_code:string|null; phase_name:string|null; owner_label:string|null; assigned_agent_id:string|null; risk_level:string|null;
  title:string; description:string|null; status:ActionStatus; priority:number; assigned_user_id:string|null;
  due_at:string|null; completed_at:string|null; created_at:string; dependencies:unknown; success_criteria:unknown; evidence_required:unknown;
};
...
async function createAction(formData:FormData){
  ...
  const {data,error}=await supabase.from("action_items").insert({organization_id:organizationId,title,description:description||null,status:"open",priority,assigned_user_id:user.id,due_at:dueAt?.toISOString()??null}).select("id").single();
  if(error||!data) redirect(`/actions?error=${encodeURIComponent(error?.message??"Action item could not be created.")}`);
  await supabase.from("audit_events").insert({organization_id:organizationId,actor_type:"user",actor_user_id:user.id,event_type:"action.created",object_type:"action_item",object_id:data.id,risk_level:priority<=2?"medium":"low",payload:{title,priority,due_at:dueAt?.toISOString()??null}});
  revalidatePath("/actions");revalidatePath("/command-center");redirect(`/actions?action=${data.id}&status=open&message=Action%20item%20created.`);
}
