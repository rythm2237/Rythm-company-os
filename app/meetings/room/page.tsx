...
async function recordCeoDecision(formData:FormData){
  ...
  const {data:meeting}=await supabase.from("meetings").select("id,status").eq("id",meetingId).eq("organization_id",organizationId).maybeSingle();
  ...
  const context=`Decision produced from governed multi-agent meeting after explicit chair closure. Question: ${session.decision_question}.${legalContext}`;
  const {data:decision,error}=await supabase.from("decisions").insert({organization_id:organizationId,project_id:session.project_id,decision_key:decisionKey,title,context,options,recommendation:session.recommendation?{text:session.recommendation}:null,rationale:`CEO selected: ${selectedOption}\n\n${rationale}`,risk_level:riskLevel,status:finalStatus,requires_human_approval:requiresApproval,decided_by_user_id:requiresApproval?null:user.id,proposed_by_agent_id:b001?.id??null,decided_at:decidedAt}).select("id").single();
  ...
  await supabase.from("meetings").update({minutes:{text:`Multi-Agent deliberation closed by Human CEO / Chair.\n\n${session.synthesis??""}${legalMinutes}\n\nHuman CEO decision: ${selectedOption}\nRationale: ${rationale}\nDecision record: ${decisionKey}`}}).eq("id",meetingId).eq("organization_id",organizationId).eq("status","completed");
  await supabase.from("audit_events").insert({organization_id:organizationId,actor_type:"user",actor_user_id:user.id,event_type:requiresApproval?"decision.created":"decision.approved",object_type:"decision",object_id:decision.id,risk_level:riskLevel,payload:{meeting_id:meetingId,session_id:sessionId,selected_option:selectedOption,human_authority:"Human CEO / Owner",chair_closed:true,legal_triage_status:session.legal_triage_status,legal_review_outcome:legalReview?.outcome??null,external_actions:false}});
