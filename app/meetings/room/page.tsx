...
export default async function Boardroom(){
  ...
  return <main className="command-shell">
    ...
    <section style={{marginTop:22,border:"1px solid #dfe4ec",borderRadius:28,background:"linear-gradient(145deg,#eef2f7,#dfe6ef)",minHeight:720,position:"relative",overflow:"hidden",boxShadow:"inset 0 1px 0 #fff,0 20px 50px rgba(25,35,58,.08)"}}>
      ...
      {agents.map((agent,i)=>{const seat=seats[i%seats.length];const src=avatarSrc(agent.avatar_url);return <Link key={agent.id} href={`/agents/${agent.agent_code.toLowerCase()}`} style={{position:"absolute",left:`${seat.x}%`,top:`${seat.y}%`,transform:"translate(-50%,0)",width:150,textAlign:"center",textDecoration:"none",color:"inherit"}}><div style={{width:104,height:104,borderRadius:"50%",overflow:"hidden",margin:"0 auto",border:"5px solid #fff",boxShadow:"0 10px 26px rgba(26,36,56,.2)",background:"#d8dee8",display:"grid",placeItems:"center",fontWeight:800}}>{src?<img src={src} alt={`${agent.display_name??agent.name} portrait`} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:agent.agent_code}</div><div style={{marginTop:8,background:"rgba(255,255,255,.92)",padding:"9px 10px",borderRadius:12,boxShadow:"0 6px 18px rgba(25,35,58,.1)"}}><strong style={{display:"block",fontSize:13}}>{agent.display_name??agent.name}</strong><span style={{display:"block",fontSize:11,color:"#687386",marginTop:3}}>{agent.role_title}</span></div></Link>})}
