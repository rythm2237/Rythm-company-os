...
create table public.organizations (
  ...
  mission text, vision text, owner_user_id uuid references auth.users(id),
...
create table public.agents (
  ...
  agent_code text not null, name text not null, role_title text not null, purpose text not null,
  authority_level smallint not null default 1 check (authority_level between 0 and 4),
  risk_ceiling public.rythm_risk_level not null default 'low', enabled boolean not null default false,
  specification_version text not null default '1.0', identity jsonb not null default '{}'::jsonb,
  permissions jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(), unique (organization_id,agent_code)
);

create table public.company_memory (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  memory_type text not null, title text not null, content jsonb not null, source_type text not null, source_id uuid,
  confidence numeric(5,4) check (confidence between 0 and 1), status public.rythm_record_status not null default 'draft',
  valid_from timestamptz not null default now(), valid_until timestamptz, supersedes_id uuid references public.company_memory(id),
...
create table public.decisions (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  decision_key text not null, title text not null, context text not null, options jsonb not null default '[]'::jsonb,
  recommendation jsonb, rationale text, risk_level public.rythm_risk_level not null default 'low',
  status public.rythm_record_status not null default 'draft', requires_human_approval boolean not null default true,
  decided_by_user_id uuid references auth.users(id), proposed_by_agent_id uuid references public.agents(id),
  decided_at timestamptz, created_at timestamptz not null default now(), unique (organization_id,decision_key)
);

create table public.approval_requests (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  subject_type text not null, subject_id uuid not null, title text not null, summary text not null,
  risk_level public.rythm_risk_level not null, requested_by_agent_id uuid references public.agents(id),
  approver_user_id uuid references auth.users(id), status public.rythm_approval_status not null default 'pending',
  conditions jsonb not null default '[]'::jsonb, response_note text, expires_at timestamptz,
  resolved_at timestamptz, created_at timestamptz not null default now()
);

create table public.meetings (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  title text not null, purpose text not null, status public.rythm_meeting_status not null default 'draft',
  scheduled_for timestamptz, started_at timestamptz, ended_at timestamptz,
  human_join_allowed boolean not null default true, agenda jsonb not null default '[]'::jsonb, minutes jsonb,
  chair_agent_id uuid references public.agents(id), created_by_user_id uuid references auth.users(id),
  created_at timestamptz not null default now()
);
...
create table public.meeting_participants (
  ...
  agent_id uuid references public.agents(id) on delete cascade, user_id uuid references auth.users(id) on delete cascade,
  attendance_status text not null default 'invited', contribution jsonb,
  check ((agent_id is not null) <> (user_id is not null))
);
create unique index meeting_participant_agent_unique on public.meeting_participants(meeting_id,agent_id) where agent_id is not null;
create unique index meeting_participant_user_unique on public.meeting_participants(meeting_id,user_id) where user_id is not null;

create table public.action_items (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  meeting_id uuid references public.meetings(id) on delete set null, decision_id uuid references public.decisions(id) on delete set null,
  title text not null, description text, status public.rythm_action_status not null default 'open',
  priority smallint not null default 3 check (priority between 1 and 5), assigned_agent_id uuid references public.agents(id),
  assigned_user_id uuid references auth.users(id), due_at timestamptz, completed_at timestamptz,
  created_at timestamptz not null default now()
);

create table public.agent_runs (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  agent_id uuid not null references public.agents(id), trigger_type text not null, input_summary text not null,
  status text not null check (status in ('queued','running','succeeded','failed','cancelled','blocked')),
  model text, input_tokens bigint not null default 0, output_tokens bigint not null default 0,
  cost_usd numeric(12,6) not null default 0, started_at timestamptz, finished_at timestamptz,
...
create table public.audit_events (
  ...
  actor_type text not null check (actor_type in ('user','agent','system')), actor_user_id uuid references auth.users(id),
  actor_agent_id uuid references public.agents(id), event_type text not null, object_type text not null, object_id text,
  risk_level public.rythm_risk_level not null default 'low', payload jsonb not null default '{}'::jsonb,
...
create policy organizations_owner_update on public.organizations for update using (public.is_org_owner(id)) with check (public.is_org_owner(id));
