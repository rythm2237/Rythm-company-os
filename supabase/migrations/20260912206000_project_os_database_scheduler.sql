...
-- Activation is explicit. Plaintext dispatcher credentials live only in Supabase Vault; the app reads only a SHA-256 hash.
...
create table if not exists public.project_scheduler_config (
  ...
  dispatcher_secret_hash text not null,
...
as $$
declare existing public.project_scheduler_config%rowtype; secret_id uuid; job_id bigint; cadence integer; secret_hash text;
begin
  ...
  secret_hash:=encode(extensions.digest(target_cron_secret,'sha256'),'hex');
  ...
  insert into public.project_scheduler_config(id,dispatcher_base_url,vault_secret_id,dispatcher_secret_hash,enabled,cadence_minutes,cron_job_id,configured_by_user_id,configured_at,updated_at)
  values(true,target_base_url,secret_id,secret_hash,true,cadence,job_id,auth.uid(),now(),now())
  on conflict(id) do update set dispatcher_base_url=excluded.dispatcher_base_url,vault_secret_id=excluded.vault_secret_id,dispatcher_secret_hash=excluded.dispatcher_secret_hash,enabled=true,cadence_minutes=excluded.cadence_minutes,cron_job_id=excluded.cron_job_id,configured_by_user_id=excluded.configured_by_user_id,configured_at=now(),updated_at=now();
...
-- Production-safe activation that never returns or exports the scheduler credential.
-- PostgreSQL/service_role generates the secret, Vault keeps plaintext, and the application gets only its hash.
create or replace function public.activate_project_os_scheduler_v1(
  target_base_url text,
  target_cadence_minutes integer default 5
)
returns jsonb
...
set search_path=public,extensions,vault,cron
as $$
declare existing public.project_scheduler_config%rowtype; generated_secret text; secret_hash text; secret_id uuid; job_id bigint; cadence integer;
begin
  ...
  secret_hash:=encode(extensions.digest(generated_secret,'sha256'),'hex');
  select * into existing from public.project_scheduler_config where id=true for update;
  if existing.cron_job_id is not null then perform cron.unschedule(existing.cron_job_id); end if;
  if existing.vault_secret_id is null then
    select vault.create_secret(generated_secret,'rythm-project-os-dispatcher','Credential for database project scheduler',null) into secret_id;
  else
    secret_id:=existing.vault_secret_id;
    perform vault.update_secret(secret_id,generated_secret,null,null,null);
  end if;
  select cron.schedule('rythm-project-os-dispatcher',format('*/%s * * * *',cadence),'select public.dispatch_project_os_from_database_v1();') into job_id;
  insert into public.project_scheduler_config(id,dispatcher_base_url,vault_secret_id,dispatcher_secret_hash,enabled,cadence_minutes,cron_job_id,configured_by_user_id,configured_at,updated_at)
  values(true,target_base_url,secret_id,secret_hash,true,cadence,job_id,null,now(),now())
  on conflict(id) do update set dispatcher_base_url=excluded.dispatcher_base_url,vault_secret_id=excluded.vault_secret_id,dispatcher_secret_hash=excluded.dispatcher_secret_hash,enabled=true,cadence_minutes=excluded.cadence_minutes,cron_job_id=excluded.cron_job_id,configured_by_user_id=null,configured_at=now(),updated_at=now();
  return jsonb_build_object('enabled',true,'cadence_minutes',cadence,'cron_job_id',job_id);
end $$;
revoke all on function public.activate_project_os_scheduler_v1(text,integer) from public,anon,authenticated;
grant execute on function public.activate_project_os_scheduler_v1(text,integer) to service_role;
...
begin
  if coalesce(auth.role(),'')<>'service_role' and current_user<>'postgres' then raise exception 'Service authorization required'; end if;
