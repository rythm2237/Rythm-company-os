...
-- External transport remains fail-closed in this MVP: no autonomous external sending.

-- Composite uniqueness allows tenant-consistent foreign keys. The global IDs remain
-- unchanged; these indexes only make organization ownership explicit at FK boundaries.
create unique index if not exists agents_organization_id_id_uidx
  on public.agents (organization_id, id);
create unique index if not exists meetings_organization_id_id_uidx
  on public.meetings (organization_id, id);
create unique index if not exists action_items_organization_id_id_uidx
  on public.action_items (organization_id, id);
...
create table if not exists public.communication_settings (
  ...
  communication_manager_agent_id uuid,
  default_approval_mode text not null default 'approval_required'
    check (default_approval_mode in ('draft_only','approval_required')),
  ...
  auto_send_enabled boolean not null default false
    check (auto_send_enabled = false),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  foreign key (organization_id, communication_manager_agent_id)
    references public.agents(organization_id, id) on delete set null
);
...
create table if not exists public.communication_mailboxes (
  ...
  approval_mode text not null default 'approval_required'
    check (approval_mode in ('draft_only','approval_required')),
...
create table if not exists public.communication_forwarding_rules (
  ...
  foreign key (organization_id, mailbox_id)
    references public.communication_mailboxes(organization_id, id) on delete cascade,
...
create table if not exists public.communication_threads (
  ...
  unique (organization_id, id),
  foreign key (organization_id, mailbox_id)
    references public.communication_mailboxes(organization_id, id) on delete set null,
  foreign key (organization_id, assigned_agent_id)
    references public.agents(organization_id, id) on delete set null,
  foreign key (organization_id, related_meeting_id)
    references public.meetings(organization_id, id) on delete set null,
  foreign key (organization_id, related_action_item_id)
    references public.action_items(organization_id, id) on delete set null
);
...
create table if not exists public.communication_messages (
  ...
  foreign key (organization_id, mailbox_id)
    references public.communication_mailboxes(organization_id, id) on delete set null,
  foreign key (organization_id, drafted_by_agent_id)
    references public.agents(organization_id, id) on delete set null
);
...
-- Organization members can read their tenant communication workspace.
drop policy if exists communication_settings_member_read on public.communication_settings;
create policy communication_settings_member_read on public.communication_settings
...
create policy communication_provider_connections_member_read on public.communication_provider_connections
for select to authenticated using (public.is_org_member(organization_id));

drop policy if exists communication_mailboxes_member_read on public.communication_mailboxes;
create policy communication_mailboxes_member_read on public.communication_mailboxes
for select to authenticated using (public.is_org_member(organization_id));

drop policy if exists communication_forwarding_rules_member_read on public.communication_forwarding_rules;
create policy communication_forwarding_rules_member_read on public.communication_forwarding_rules
for select to authenticated using (public.is_org_member(organization_id));

drop policy if exists communication_threads_member_read on public.communication_threads;
create policy communication_threads_member_read on public.communication_threads
for select to authenticated using (public.is_org_member(organization_id));

drop policy if exists communication_messages_member_read on public.communication_messages;
create policy communication_messages_member_read on public.communication_messages
for select to authenticated using (public.is_org_member(organization_id));
...
create policy communication_settings_owner_manage on public.communication_settings
...
create policy communication_provider_connections_owner_manage on public.communication_provider_connections
for all to authenticated
using (public.is_org_owner(organization_id))
with check (public.is_org_owner(organization_id));

drop policy if exists communication_mailboxes_owner_manage on public.communication_mailboxes;
create policy communication_mailboxes_owner_manage on public.communication_mailboxes
for all to authenticated
using (public.is_org_owner(organization_id))
with check (public.is_org_owner(organization_id));

drop policy if exists communication_forwarding_rules_owner_manage on public.communication_forwarding_rules;
create policy communication_forwarding_rules_owner_manage on public.communication_forwarding_rules
for all to authenticated
using (public.is_org_owner(organization_id))
with check (public.is_org_owner(organization_id));

drop policy if exists communication_threads_owner_manage on public.communication_threads;
create policy communication_threads_owner_manage on public.communication_threads
for all to authenticated
using (public.is_org_owner(organization_id))
with check (public.is_org_owner(organization_id));

drop policy if exists communication_messages_owner_manage on public.communication_messages;
create policy communication_messages_owner_manage on public.communication_messages
for all to authenticated
using (public.is_org_owner(organization_id))
with check (public.is_org_owner(organization_id));
...
-- Provision the mailbox identity immediately when an organization is created. Do not
-- create an agent here: commercial entitlement is inserted later in the existing
-- customer provisioning transaction and the agent entitlement guard would reject it.
...
-- Create/attach the Communication Manager only when the organization's commercial
-- entitlement is active. This preserves the existing entitlement guard on agents.
create or replace function public.provision_communication_manager_for_org(target_org_id uuid)
returns uuid
language plpgsql
security definer
set search_path = public
...
begin
  ...
  set role_title = excluded.role_title,
      ...
      human_approval_requirements = excluded.human_approval_requirements,
  ...
  set communication_manager_agent_id = v_manager_agent_id,
      updated_at = now()
  where organization_id = target_org_id;

  update public.communication_mailboxes
  set assigned_agent_id = v_manager_agent_id,
      updated_at = now()
  where organization_id = target_org_id
    and assigned_agent_id is null
    and mailbox_type = 'system';

  return v_manager_agent_id;
end;
$$;

revoke all on function public.provision_communication_manager_for_org(uuid) from public, anon, authenticated;

create or replace function public.provision_communication_manager_on_entitlement()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if new.status = 'active' and (tg_op = 'INSERT' or old.status is distinct from new.status) then
    perform public.provision_communication_manager_for_org(new.organization_id);
  end if;
  return new;
end;
$$;

revoke all on function public.provision_communication_manager_on_entitlement() from public, anon, authenticated;

drop trigger if exists organization_entitlement_provision_communication_manager on public.organization_entitlements;
create trigger organization_entitlement_provision_communication_manager
after insert or update of status on public.organization_entitlements
for each row execute function public.provision_communication_manager_on_entitlement();

-- Backfill managed workspace identity for existing organizations.
insert into public.communication_settings (
  organization_id,
  managed_subdomain,
  managed_domain,
...
select o.id, o.slug, 'rythm-os.com', null, 'approval_required', 'high', false
...
-- Existing active organizations receive the governed system agent immediately.
do $$
declare
  v_org record;
begin
  ...
  loop
    perform public.provision_communication_manager_for_org(v_org.id);
  end loop;
end;
$$;

comment on table public.communication_provider_connections is
  'Provider-neutral transport metadata. Secrets and OAuth tokens are intentionally not stored in this table.';
comment on column public.communication_settings.auto_send_enabled is
  'Fail-closed by database constraint in the Communication MVP. A future governed migration is required before autonomous external sending can be enabled.';
