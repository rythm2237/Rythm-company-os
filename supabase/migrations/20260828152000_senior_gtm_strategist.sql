...
select
  ...
  '["Define market and commercial opportunity while separating facts, assumptions and hypotheses","Develop transparent TAM, SAM and SOM estimates when evidence supports market sizing","Prioritize customer segments and define ICP, buyer, user, influencer and decision-maker roles","Create positioning, value proposition, differentiation and structured messaging frameworks","Assess offer, packaging, pricing and revenue-model hypotheses and escalate financial or legal review where required","Evaluate owned, earned, paid, partner, outbound and sales-led channels by reach, cost, speed, control, risk and learning value","Build phased pre-launch, launch and post-launch plans including 30/60/90-day roadmaps","Design funnel stages, KPIs and experiments appropriate to the business model","Align marketing promises, sales handoffs, enablement assets and delivery capacity","Own the strategic GTM plan while delegating specialist execution through governed RYTHM handoffs"]'::jsonb,
  '["go-to-market strategy","market opportunity analysis","TAM SAM SOM","customer segmentation","ICP development","positioning","value proposition","messaging strategy","pricing and packaging hypotheses","channel strategy","launch planning","growth experimentation","funnel design","KPI design","sales and marketing alignment","scenario analysis","executive recommendation","risk and evidence assessment"]'::jsonb,
  ...
  base.default_allowed_tools,base.default_memory_scope,base.default_language,
  $gtm$
...
ROUTING AND TOOLS
Do not select or call an AI provider directly. Model/tier selection is delegated to RYTHM Request Intelligence, Adaptive Routing and the AI Gateway. Use only permitted least-privilege tools. If evidence/tool access fails, return a clear partial-result state; never fabricate missing evidence.
$gtm$,
...
on conflict (template_key,version) do update set
  name=excluded.name,role=excluded.role,role_code=excluded.role_code,department_template_key=excluded.department_template_key,
  reports_to_template_key=excluded.reports_to_template_key,purpose=excluded.purpose,responsibilities=excluded.responsibilities,
  skills=excluded.skills,work_style=excluded.work_style,kpis=excluded.kpis,success_criteria=excluded.success_criteria,
  default_authority_level=excluded.default_authority_level,default_risk_ceiling=excluded.default_risk_ceiling,
  default_human_approval_requirements=excluded.default_human_approval_requirements,default_allowed_tools=excluded.default_allowed_tools,
  default_memory_scope=excluded.default_memory_scope,default_language=excluded.default_language,
  system_instructions_template=excluded.system_instructions_template,runtime_policy_key=excluded.runtime_policy_key,
  budget_policy_key=excluded.budget_policy_key,is_active=true,canonical_role=excluded.canonical_role,
  role_family=excluded.role_family,default_specializations=excluded.default_specializations,
...
set agent_template_refs = case when 'gtm-strategist'=any(agent_template_refs) then agent_template_refs else array_append(agent_template_refs,'gtm-strategist') end,
    department_templates = (
      select jsonb_agg(case when item->>'key'='strategy' then item || jsonb_build_object('name','Strategy & Growth','description','Advertising strategy, GTM, positioning, market entry and executive growth coordination') else item end order by ord)
...
-- GTM remains advisory: give it read-only strategy inputs. Execution stays delegated to specialist Agents.
...
values
  ('ready_ai_advertising_agency_v1','gtm-strategist','crm_sales','records.read','autonomous','{"tenant_only":true,"strategy_input_only":true}'::jsonb),
  ('ready_ai_advertising_agency_v1','gtm-strategist','meta_marketing','campaign.read','autonomous','{"tenant_only":true,"strategy_input_only":true}'::jsonb),
  ('ready_ai_advertising_agency_v1','gtm-strategist','google_ads','campaign.read','autonomous','{"tenant_only":true,"strategy_input_only":true}'::jsonb),
  ('ready_ai_advertising_agency_v1','gtm-strategist','tiktok_business','campaign.read','autonomous','{"tenant_only":true,"strategy_input_only":true}'::jsonb),
  ('ready_ai_advertising_agency_v1','gtm-strategist','linkedin_marketing','campaign.read','autonomous','{"tenant_only":true,"strategy_input_only":true}'::jsonb),
  ('ready_ai_advertising_agency_v1','gtm-strategist','youtube','channel.read','autonomous','{"tenant_only":true,"strategy_input_only":true}'::jsonb)
...
begin
  ...
  where a.template_key='gtm-strategist' and a.version='1.0' and a.is_active=true
    and a.role_family='marketing' and a.canonical_role='Senior GTM Strategist'
    and a.runtime_policy_key is not null
    and exists (select 1 from public.role_foundations rf where rf.role_family=a.role_family and rf.status in ('active','validated') and (rf.canonical_role=a.canonical_role or rf.canonical_role is null))
    and not exists (
      select 1 from unnest(a.default_specializations) s(specialization_key)
      where not exists (select 1 from public.role_specializations rs where rs.role_family=a.role_family and rs.specialization_key=s.specialization_key and rs.active=true)
    );
  if v_count <> 1 then raise exception 'Senior GTM Strategist canonical professional contract is incomplete'; end if;

  if not exists (
    select 1 from public.company_templates c
    where c.template_key='ready_ai_advertising_agency_v1' and c.version='1.0'
      and 'gtm-strategist'=any(c.agent_template_refs) and (c.launch_configuration->>'agent_count')::integer=11
  ) then raise exception 'Senior GTM Strategist is not correctly associated with the Advertising Agency'; end if;

  select count(*) into v_unrelated from public.company_templates c
  where c.template_key <> 'ready_ai_advertising_agency_v1' and 'gtm-strategist'=any(c.agent_template_refs);
  if v_unrelated <> 0 then raise exception 'Senior GTM Strategist must not be auto-added to unrelated Ready Companies'; end if;
end $$;
